...
class ArticleRepository implements IArticleRepository{
  ...
  final Box<List<Article>> _cachedArticleBox;
  final SharedPreferences _sharedPreferences;
  final FirebaseMessaging _firebaseMessaging;
  ArticleRepository(@Named("favArticleUidBox") this._favArticleBox,
      @Named("cachedArticleBox") this._cachedArticleBox, this._sharedPreferences, this._firebaseMessaging);

  @override
  Future<Either<ArticleFailure, List<Article>>> getArticles(String category) async{
    try {
      if(kIsWeb||(await InternetConnectionChecker().hasConnection)){
        final response = await http.get(Uri.parse('https://api.nytimes.com/svc/topstories/v2/$category.json?api-key=vbxqbDJG068DqTLicxuJpWfAHgi15cQz'));
        if (response.statusCode == 200) {
          final List<Article> articles =ArticleCollection.fromJson(jsonDecode(utf8.decode(response.bodyBytes))).results;
          await cacheArticles(articles, category);
          return right(articles);
        } else if(response.statusCode==429){
          final List<Article> articles =_cachedArticleBox.get(category)??[];
          return right(articles);
        }else {
 ...
 void _unsubscribeFromTopic(String topicName){
  ...
  async{
    try {
          await _cachedArticleBox.put(category,articles);
      return right(unit);
    } on HiveError {
  ...
  @override
  Future<Either<ArticleFailure, Unit>> createFavoriteTopic(ArticleCategoryEnum categoryEnum) async{
    try {
  ...
  @override
  Future<Either<ArticleFailure, Unit>> deleteFavoriteTopic(ArticleCategoryEnum categoryEnum)async {
    try {
      final favoritesEither = await getFavoriteTopics();
      return favoritesEither.fold((failure) {
        return left(failure);
  ...
  @override
  Future<Either<ArticleFailure, Unit>> createFavoriteArticle(Article article) async{
    try {
  ...
  @override
  Future<Either<ArticleFailure, Unit>> deleteFavoriteArticle(Article article) async{
    try {
      final favoritesEither = await getFavoriteArticleUrls();
      return favoritesEither.fold((failure) {
        return left(failure);
